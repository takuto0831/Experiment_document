...
[クラス間マージンなどについて](https://www.slideshare.net/moa108/8-28571831)
...
`Pima.tr`との`glu`と`bmi`を用いた2次元データをSVMで識別してみる. RBFカーネルを用いた結果を示す. RBFカーネルにはコストパラメータ$C$とカーネルパラメータ$\sigma$が存在する.動径基底関数の広がりを表すパラメータを$\sigma=0.2, 0.4, 0.8, 2.0,4.0$に設定し, $\alpha_i$の上限値を与えるパラメータ$C$の値を変えたときの訓練誤差(学習データの再代入誤り率)と,  ホールドアウト法による汎化誤差を示している. （教科書と違います）　
...
pred.te <- predict(pima.model,Pima.te) #テストデータに対する実行
...
今回の例では`Pima.tr`データの7変数を用いて, ロジスティック回帰を行なったが, 幾つの変数を用いるべきか考察せよ.
...
今回の例では`Pima.tr`の`glu`と`bmi`の2変数を用いて, サポートベクトルマシンによるニ値分類を行なったが, 他の変数を用いて同様の検証を行い, 考察せよ. その際どの変数, パラメータを用いたかを記載し, 必ず理由を記載すること.
...
ロジスティック回帰およびサポートベクトルマシンについて, 調べたことなどを書いてください.
