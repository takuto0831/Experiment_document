...
[ksvm関数について](http://uc-r.github.io/svm)
...
```{r}
pima.model_test <- ksvm(type~ bmi+glu,data=train[pima.model@SVindex,],
                        type="C-svc",kernel=rbf,C=10,prob.model=TRUE)
plot(pima.model_test,data = train[pima.model@SVindex,])
```
...
t(result) %>% 
  ...
  gather(key = "key", value = "value", -id) -> aaa
aaa$value <- unlist(aaa$value)

ggplot(aaa,aes(x=id, y=value, group = key, colour = key)) +
  geom_point() +
