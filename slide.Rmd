...
# windows用
# setwd("C:/Users/ms/Desktop/情報工学実験３")
```
...
学習データから, 誤り率と分類評価指標を求めてみよう. 元データの数値を用いて, 得られたパラメータにより線形識別関数を計算する.
...
全てのデータにおける"type"を"No"と仮定し, 線形識別関数を用いて0.5より大きい場合を"Yes"とする. 横軸を真値, 縦軸を予測値としてクロス集計表を作成する.
...
ROC曲線は, "yes","no"が入り混じることなく現れるデータの場合どのような曲線になるでしょう？ 

一般的に, 予測の正答率を上げていくために, 閾値を移動させると, 誤判別も増えて行きます. 

# サポートベクトルマシン

## サポートベクトルマシンのアルゴリズム(1/4)
...
## サポートベクトルマシンのアルゴリズム(2/4)

ここで,先ほど示した点と平面の公式の分母である$\Vert  \bf{w} \Vert$は両辺にかけて消去している, その結果得られた値をマージン$\kappa$としている.
...
## サポートベクトルマシンのアルゴリズム(3/4)

この場合分けは$t_i( \bf{w}^T \bf{x}_i + b ) \geq 1$とまとめることができる. クラス間マージンは, 各クラスのデータを$\bf{w}$の方向へ射影した長さの差の最小値(つまり$t_i = +1$のとき$\bf{w}^T \bf{x}_i  + b = 1$となり, $t_i = -1$のとき$\bf{w}^T \bf{x}_i  + b = -1$となるとき)
...
動径基底関数を作成し, サポートベクトルマシンによる分類モデルを作る. 動径基底関数とはカーネル関数の一種で, 入力空間のベクトルを特徴空間のベクトルに変換する.

[カーネル関数について](https://qiita.com/sergeant-wizard/items/c5f79adefe5016f4740e)

```{r ex8.1.a}
rbf <- rbfdot(sigma=0.1)
pima.model <- ksvm(type~ bmi+glu ,data=train,type="C-svc",kernel=rbf,C=10,prob.model=TRUE)
```
...
"result"にまとめた, 各結果を番号順にプロットする. 奇数番号はテストデータに対する予測なので全体的に悪くなっている. 
...
訓練誤差が最小の場合, 汎化誤差が最小の場合についてクロス集計表を作成する.
